...
public class AnimationUtil {
     ...
     * @param Duration 动画时间
     */
    public static void moveToViewBottom_Gone(final View v, long Duration) {
     ...
     * @param Duration 动画时间
     */
    public static void bottomMoveToViewLocation_Visible(View v, long Duration) {
        if (v.getVisibility() == View.VISIBLE)
            return;
     ...
     * @param Duration 动画时间
     */
    public static void moveToViewTop_Gone(final View v, long Duration) {
        if (v.getVisibility() != View.VISIBLE)
            return;
        if (ismHiddenActionstart)
            return;
     ...
     * @param Duration 动画时间
     */
    public static void topMoveToViewLocation_Visible(View v, long Duration) {
        if (v.getVisibility() == View.VISIBLE)
            return;
        v.setVisibility(View.VISIBLE);
        TranslateAnimation mShowAction = new TranslateAnimation(Animation.RELATIVE_TO_SELF, 0.0f,
